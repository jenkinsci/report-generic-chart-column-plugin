...
# Callig other deffined functions
Instead of writing expression to the equation line, you may write other preset function with parameters:
```
 "equations": [
      {
        "name": "call1",
...
```
 * the equation in this scope must be single lined (or at least sane when lines are connected)
 * otherhappening should be natural
   * no escapes from called function out
   * correct trasnformation of parameters to internal params
   * calls acan be chained
# full example
```
[... },
  {
